...
class CenterLoss(nn.Module):
    ...
    def forward(self, x, labels):
        ...
        center_loss = (x - expanded_centers).pow(2).sum() / 2.0 / batch_size
        return center_loss
#
#class CombinedLoss(nn.Module):
#    def __init__(self, num_classes, feat_dim, device='cpu', lambda_center=0.05):
#        super(CombinedLoss, self).__init__()
#        self.center_loss = CenterLoss(num_classes, feat_dim, device)
#        self.cross_entropy_loss = nn.CrossEntropyLoss()
#        self.lambda_center = lambda_center  # Weight for the center loss
#
#    def forward(self, features, labels, logits):
#        ce_loss = self.cross_entropy_loss(logits, labels)
#        cl_loss = self.center_loss(features, labels)
#        total_loss = ce_loss + self.lambda_center * cl_loss
...
class CombinedLoss(nn.Module):
    def __init__(self, num_classes, feat_dim, device):
        ...
        self.center_loss = CenterLoss(num_classes, feat_dim, device)  # Assuming you have a CenterLoss class defined

    def forward(self, features, labels, logits):
        ce_loss = self.cross_entropy(logits, labels)
        cl_loss = self.center_loss(features, labels)
        loss = ce_loss + cl_loss
        return loss, ce_loss, cl_loss
